Global $hAPSWinAbout, $hAPSWinAboutHyperlink, $hAPSWinAboutCloseBtn

Func DialogAbout()
	GUISetState(@SW_DISABLE, $hTTWinMain)
	$hAPSWinAbout = GUICreate("About Android ProShop", 404, 470, -1, -1, $WS_POPUP + $WS_BORDER, -1, $hTTWinMain)
		setIcon(1)
		...
		$hAPSWinAboutHyperlink = GUICtrlCreateLabel("Created by CosmicDan", 0, 160, 404, -1, $SS_Center)
			GuiCtrlSetFont($hAPSWinAboutHyperlink, -1, -1, 4) ; underlined
			GuiCtrlSetColor($hAPSWinAboutHyperlink,0x0000ff)
			GuiCtrlSetCursor($hAPSWinAboutHyperlink,0)
			GUICtrlSetOnEvent($hAPSWinAboutHyperlink, "APSWinAboutEvent")
		$hAPSWinAboutCloseBtn = GUICtrlCreateButton ("Close", 306, 432, 92, 32)
			GUICtrlSetOnEvent (-1, "APSWinAboutEvent")
	...
	GUISetState(@SW_SHOW, $hAPSWinAbout)
EndFunc

Func APSWinAboutEvent()
	Switch @GUI_CtrlId
		Case $hAPSWinAboutCloseBtn
			GUIDelete ($hAPSWinAbout)
			GUISetState(@SW_ENABLE, $hTTWinMain)
			GUISwitch($hTTWinMain)
			WinActivate($sSysTitle)
		Case $hAPSWinAboutHyperlink
