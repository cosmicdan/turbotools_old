...
;
;   NOTE: Does not work in Windows XP, it will fail with an error.
;
    ...
    For $oItem In $oItems
        If $oItem.IsFolder Then
            ...
            Else
                $sSub &= "\" & $oItem.Name
            EndIf
            $aCount = _Zip_CountAll($sZipFile, $sSub, $iFileCount, $iFolderCount)
            $iFileCount = $aCount[0]
    ...
    For $oItem In $oList
        If $oItem.IsFolder Then
            ...
            If $iFullPath Then
                ...
                _Zip_ListAll_Internal($sZipFile, $sSub, $aArray, $iFullPath, $sSub & "\")
                If @error Then Return SetError(4)
            Else
                ; just filenames
                _Zip_ListAll_Internal($sZipFile, $sSub, $aArray, $iFullPath, "")
                If @error Then Return SetError(4)
            EndIf
        Else
            $aArray[0] += 1
